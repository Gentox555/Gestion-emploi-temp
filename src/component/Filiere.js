...
const Filiere = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    fetch('http://localhost/project-gestion/Backend/Filiere/ajouter.php', {
      method: 'POST',
      headers: {
    ...
    .then(data => {
      if (data.status === 'success') {
        setfiliere([...filiere, { ...formData, id: data.id }]);
        setShowForm(false);
        setFormData({
          nom: '',
          dateCreation: '',
    ...
    .catch(error => {
      ...
      alert('Une erreur est survenue lors de l\'ajout du filiére.');
    });
  };
  // Fetch existing departments
  useEffect(() => {
    fetch('http://localhost/project-gestion/Backend/Filiere/recup.php')
      .then(response => response.json())
      .then(data => {
        if (data.status === 'success') {
          setfiliere(data.filière);
        } else {
  ...
  return (
    <div className="">
      ...
      <div className="text-center text-xl font-semibold text-gray-700 translate-y-32 -translate-x-80 z-20">Filiére</div>
      <span 
        className="absolute border-1 top-40 rounded w-10 h-10 bg-blue-600 z-50 right-5 cursor-pointer"onClick={() => setShowForm(!showForm)}>
      ...
      <div className=" translate-x-96 ml-56 translate-y-72 border-2 w-min border-transparente overflow-y-auto h-72 ">
        <table className="divide-y divide-gray-200 ">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filiere.map((fil) => (
              <tr key={fil.id}>
                <td className="px-6 py-4 whitespace-nowrap">{fil.nom}</td>
                <td className="px-6 py-4 whitespace-nowrap">{fil.datefi}</td>
      ...
      <div className="-translate-y-80">
        <ul className="mt-40 ml-12 p-4 space-y-14">
          ...
          <li className="relative text-lg font-semibold text-gray-700  w-40 hover:text-blue-600 transition-all duration-200 cursor-pointer group">
            <span className="absolute inset-y-0 -ml-16 w-2 h-10 -translate-y-1 bg-transparent rounded-r transition-all duration-200 group-hover:bg-blue-600"></span>
            <span className="pl-4 group-hover:underline">Filiére</span>
            <div className="-translate-x-4 -translate-y-6">
      ...
      {showForm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-8 rounded shadow-md w-96">
            <h2 className="text-2xl mb-4">Ajouter Filière</h2>
